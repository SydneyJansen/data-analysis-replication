...
```{r }
file<- "https://github.com/SydneyJansen/data-analysis-replication/blob/main/data/rawdata.csv"
data <- read.csv(file)
...
```{r out.width="400px", fig.cap="Figure x. x"}
knitr::include_graphics("https://github.com/x")
```
...
ggplot(permuted_data, aes(x = factor(treatment))) +
  
  # Add points for bootstrapped median
  geom_point(aes(y = boot_median), size = 3) +
  
  # Add error bars for bootstrapped quartiles
  geom_errorbar(
  ...
  labs(x = "Treatment", y = "Median Call Duration (ms)", title = "Bootstrapped Median Call Duration vs. Treatment") +
